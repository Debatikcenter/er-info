...
d3.json(data, function (error, graph) {
  ...
  simulation = d3
    ...
    ) //if(query!="") return -800; else if(projectQuery != "") return -2000; else return -150;
  ...
  updateGraph = function (year) {
    ...
    var projects = $.grep(json.projects, function (n, i) {
      return (
        n.start.substring(0, 4) <= year &&
        (n.end.substring(0, 4) >= year || n.end == "")
      );
    });
    ...
    $.grep(circles, function (n) {
      return uRelPersons.includes(n.id);
    }).forEach(function (n) {
      n.colorize();
    });
    $.grep(circles, function (n) {
    ...
    $.grep(lines, function (n) {
      return (
        $.inArray(n.source.id, uRelPersons) != -1 &&
        $.inArray(n.target.id, uRelPersons) != -1
      );
    }).forEach(function (n) {
      n.colorize();
    });
...
d3.select("canvas").on("click", function (d) {
  ...
  if (
    (p[0] - closeNode.x) * (p[0] - closeNode.x) +
      (p[1] - closeNode.y) * (p[1] - closeNode.y) <
      (5 + parseInt(closeNode.nr_projects)) *
        (5 + parseInt(closeNode.nr_projects)) &&
    circles[closeNode.index].opacity != 0
  ) {
    // alert(closeNode)
